...
static int parse_uint8_fastswar_bob(const char *str, size_t len, uint8_t *num) {
  ...
  *num = (uint8_t)((0x640a01 * digits) >> 16);
  uint32_t all_digits = ((digits | (digits + 0x767676)) & 0x808080) == 0;
  return all_digits && ((len ^ 3) < 3) && __builtin_bswap32(digits) <= 0x020505ff;
}

typedef int fun_t(const char*, size_t, uint8_t*);
...
static void test(fun_t* fun, const char* name) {
  printf("testing %s\n", name);
  test_t t = {fun, {0}, 0, 0};
  test_rec(&t);
}
